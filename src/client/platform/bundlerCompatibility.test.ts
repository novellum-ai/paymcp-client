...
describe("Bundler Compatibility", () => {
  it("should use eval trick for better-sqlite3 to prevent bundler analysis", () => {
    ...
    if (!existsSync(join(__dirname, "../../../dist/src/platform/index.js"))) {
      console.log(
        "Skipping test because dist/src/platform/index.js does not exist",
      );
      return;
    }
    ...
    const compiledPath = join(
      __dirname,
      "../../../dist/src/client/platform/index.js",
    );
  ...
  it("should not have direct Node.js module imports in platform code", () => {
    // Skip this test if the file does not exist
    if (
      !existsSync(join(__dirname, "../../../dist/src/client/platform/index.js"))
    ) {
      console.log(
        "Skipping test because dist/src/client/platform/index.js does not exist",
      );
      return;
    }
    ...
    for (const builtin of nodeBuiltins) {
      expect(compiledCode).not.toMatch(
