...
describe("payMcpClient.fetch oauth", () => {
  it("should auth using the payment maker if one is available", async () => {
    ...
    const paymentMaker = {
      ...
      }) => Promise.resolve(JSON.stringify(params)),
    };
    ...
    const authCall = f.callHistory.lastCall(
      `begin:${DEFAULT_AUTHORIZATION_SERVER}/authorize`,
    );
  ...
  it("should auth using the local token if one is available", async () => {
    ...
    await db.saveAccessToken("bdj", "", {
      accessToken: "testAccessToken",
      resourceUrl: "https://my-current-url.com",
    });
    mockResourceServer(
      f,
      "https://example.com",
    ...
    const originalFromDb = await db.getAccessToken(
      "bdj",
    ...
    // Ensure we call the authorize endpoint
    const downstreamCalls = f.callHistory.callLogs.filter(
      (call) => call.url == "https://example.com/mcp",
    );
  ...
  it("should bubble up OAuthAuthenticationRequiredError on OAuth challenge with no paymentMakers or local token", async () => {
    ...
    const fetcher = payMcpFetcher(f.fetchHandler, {});
    await expect(fetcher.fetch("https://example.com/mcp")).rejects.toThrow(
  ...
  it("should throw if authorization server response is not successful", async () => {
    ...
    await expect(fetcher.fetch("https://example.com/mcp")).rejects.toThrow(
      "Expected redirect response from authorization URL, got 401",
    );
  });
    ...
    mockAuthorizationServer(f, DEFAULT_AUTHORIZATION_SERVER)
      ...
      .get(`begin:${DEFAULT_AUTHORIZATION_SERVER}/authorize`, (req) => {
        ...
        return {
          ...
          headers: {
            location: `paymcp://paymcp?state=${state}&error=invalid_request`,
          },
        };
      });
