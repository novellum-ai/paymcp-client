...
} from "../common/paymentRequiredError.js";
import {
  AccessToken,
...
export class PayMcpFetcher {
  ...
  constructor({
    ...
    const safeFetchFn = getIsReactNative()
      ? createReactNativeSafeFetch(fetchFn)
      : fetchFn;
    const safeSideChannelFetch = getIsReactNative()
      ? createReactNativeSafeFetch(sideChannelFetch)
      : sideChannelFetch;
  ...
  protected handlePaymentRequestError = async (
    paymentRequestError: McpError,
  ): Promise<boolean> => {
    if (paymentRequestError.code !== PAYMENT_REQUIRED_ERROR_CODE) {
      throw new Error(
        `PayMCP: expected payment required error (code ${PAYMENT_REQUIRED_ERROR_CODE}); got code ${paymentRequestError.code}`,
      );
    }
    const paymentRequestUrl = (
      paymentRequestError.data as { paymentRequestUrl: string } | undefined
    )?.paymentRequestUrl;
    if (!paymentRequestUrl) {
      throw new Error(
        `PayMCP: payment requirement error does not contain a payment requirement URL`,
      );
    }
    const paymentRequestId = (
      paymentRequestError.data as { paymentRequestId: string } | undefined
    )?.paymentRequestId;
    if (!paymentRequestId) {
      throw new Error(
        `PayMCP: payment requirement error does not contain a payment request ID`,
      );
    }
    if (!this.isAllowedAuthServer(paymentRequestUrl)) {
      this.logger.info(
        `PayMCP: payment requirement is not allowed on this server`,
      );
      return false;
    }
    ...
    if (!prRequest.ok) {
      throw new Error(
        `PayMCP: GET ${paymentRequestUrl} failed: ${prRequest.status} ${prRequest.statusText}`,
      );
    }
    ...
    if (!paymentMaker) {
      this.logger.info(
        `PayMCP: payment network ${requestedNetwork} not set up for this server (available networks: ${Array.from(this.paymentMakers.keys()).join(", ")})`,
      );
      return false;
    }
    ...
    const paymentId = await paymentMaker.makePayment(
      amount,
      currency,
      destination,
      paymentRequest.iss,
    ...
    this.logger.info(
      `PayMCP: made payment of ${amount} ${currency} on ${requestedNetwork}: ${paymentId}`,
    );
    ...
    this.logger.debug(
      `PayMCP: payment was ${response.ok ? "successfully" : "not successfully"} PUT to ${paymentRequestUrl} : status ${response.status} ${response.statusText}`,
    );
  ...
  protected makeAuthRequestWithPaymentMaker = async (
    oauthError: OAuthAuthenticationRequiredError,
    ...
    if (!this.isAllowedAuthServer(authorizationUrl.toString())) {
      throw new Error(
        `PayMCP: authorization server ${oauthError.url} is not allowed`,
      );
    }

    const authToken = await paymentMaker.generateJWT({
      paymentRequestId: "",
    ...
    // if we pass redirect=false.
    // TODO: Remove the redirect=false hack once we have a way to handle the redirect on mobile
    const response = await this.sideChannelFetch(
      authorizationUrl.toString() + "&redirect=false",
      {
        method: "GET",
        redirect: "manual",
        headers: {
    ...
    if (response.status >= 300 && response.status < 400) {
      ...
      if (location) {
        this.logger.info(
          `PayMCP: got redirect authorization code response - redirect to ${location}`,
        );
        return location;
      } else {
        this.logger.info(
          `PayMCP: got redirect authorization code response, but no redirect URL in Location header`,
        );
      }
    }
    ...
    if (response.ok) {
      ...
      if (redirectUrl) {
        this.logger.info(
          `PayMCP: got response.ok authorization code response - redirect to ${redirectUrl}`,
        );
        return redirectUrl;
      } else {
        this.logger.info(
          `PayMCP: got authorization code response with response.ok, but no redirect URL in body`,
        );
      }
    }
    ...
    throw new Error(
      `Expected redirect response from authorization URL, got ${response.status}`,
    );
  };

  protected authToService = async (
    error: OAuthAuthenticationRequiredError,
  ): Promise<void> => {
    ...
    if (this.paymentMakers.size > 1) {
      throw new Error(
        `PayMCP: multiple payment makers found - cannot determine which one to use for auth`,
      );
    }
    ...
    if (paymentMaker) {
      ...
      const redirectUrl = await this.makeAuthRequestWithPaymentMaker(
        error,
        paymentMaker,
      );
      ...
      if (!existingToken) {
        this.logger.info(
          `PayMCP: no token found for the current server - we can't exchange a token if we don't have one`,
        );
        throw error;
      }
  ...
  protected checkForPayMcpResponse = async (
    response: Response,
  ): Promise<void> => {
    ...
    try {
      ...
      paymentRequests = messages
        .flatMap((message) => parsePaymentRequests(message))
        .filter((pr) => pr !== null);
    } catch (error) {
      this.logger.error(
        `PayMCP: error checking for payment requirements in MCP response: ${error}`,
      );
    ...
    if (paymentRequests.length > 1) {
      throw new Error(
        `PayMCP: multiple payment requirements found in MCP response. The client does not support multiple payment requirements. ${paymentRequests.map((pr) => pr.url).join(", ")}`,
      );
    }
    for (const { url, id } of paymentRequests) {
      this.logger.info(
        `PayMCP: payment requirement found in MCP response - ${url} - throwing payment required error`,
      );
  ...
  fetch: FetchLike = async (url, init) => {
    ...
    try {
      ...
      if (error instanceof OAuthAuthenticationRequiredError) {
        this.logger.info(
          `OAuth authentication required - PayMCP client starting oauth flow for resource metadata ${error.resourceServerUrl}`,
        );
      ...
      if (mcpError && mcpError.code === PAYMENT_REQUIRED_ERROR_CODE) {
        if (await this.handlePaymentRequestError(mcpError)) {
          ...
          this.logger.info(
            `PayMCP: payment request was not completed successfully`,
          );
        }
        if (response) {
          return response;
