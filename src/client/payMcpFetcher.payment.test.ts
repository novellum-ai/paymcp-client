...
describe("payMcpClient.fetch payment", () => {
  it("should make a payment if the server response is a paymcp payment request error", async () => {
    ...
    const errTxt = CTH.paymentRequiredMessage(
      DEFAULT_AUTHORIZATION_SERVER,
      "foo",
    );
    const errMsg = CTH.mcpToolErrorResponse({
      content: [{ type: "text", text: errTxt }],
    });
    ...
    const payCall = f.callHistory.lastCall(
      `begin:${DEFAULT_AUTHORIZATION_SERVER}/payment-request/foo`,
    );
  ...
  it("should make a payment if the server response is an elicitation request error", async () => {
    const f = fetchMock.createInstance();
    const errMsg = CTH.mcpElicitationRequiredErrorResponse({
      url: `${DEFAULT_AUTHORIZATION_SERVER}/payment-request/foo`,
      elicitationId: "foo",
    });
  ...
  it("should pass through an elicitation request error that is not paymcp", async () => {
    const f = fetchMock.createInstance();
    const errMsg = CTH.mcpElicitationRequiredErrorResponse({
      url: `https://slack.com/give-me-api-key`,
      elicitationId: "foo",
    });
  ...
  it("should retry the request if a payment was required and made successfully", async () => {
    ...
    const mcpCalls = f.callHistory.callLogs.filter((call) =>
      call.url.startsWith("https://example.com/mcp"),
    );
  ...
  it("should pass through a payment request response if there is no matching payment maker", async () => {
    ...
    expect(resJson.result.content[0].text).to.include(
      "Payment via PayMcp is required",
    );
    expect(resJson.result.content[0].text).to.include(
      `${DEFAULT_AUTHORIZATION_SERVER}/payment-request/foo`,
    );
  });
  ...
  it("should throw an error if the server does not have the payment request", async () => {
    ...
    mockAuthorizationServer(f, DEFAULT_AUTHORIZATION_SERVER, {}).getOnce(
      `${DEFAULT_AUTHORIZATION_SERVER}/payment-request/foo`,
      404,
    );
    let threw = false;
  ...
  it("should pass through payment request response if the payment request server is not allowed", async () => {
    ...
    const fetcher = payMcpFetcher(
      f.fetchHandler,
      { solana: paymentMaker },
      undefined,
      ["https://not-paymcp.com"],
    );
    const res = await fetcher.fetch("https://example.com/mcp");
  ...
  it("should throw an error if PUTing to the payment-request endpoint fails", async () => {
    ...
    mockAuthorizationServer(f, DEFAULT_AUTHORIZATION_SERVER, {}).putOnce(
      `${DEFAULT_AUTHORIZATION_SERVER}/payment-request/foo`,
      500,
    );
    let threw = false;
