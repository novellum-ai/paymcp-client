...
function oauthClient(
  ...
  strict: boolean = true,
  callbackUrl: string = "https://example.com/mcp/callback",
) {
...
describe("oauthClient", () => {
  describe(".fetch", () => {
    ...
    it("should throw OAuthAuthenticationRequiredError with authorization url on OAuth challenge", async () => {
      ...
      const client = oauthClient(f.fetchHandler);
      await expect(client.fetch("https://example.com/mcp")).rejects.toThrow(
    ...
    it("should throw OAuthAuthenticationRequiredError with resource server url from www-authenticate header for old-style proxied requests", async () => {
      ...
      const f = fetchMock.createInstance().getOnce("https://example.com/mcp", {
        ...
        headers: {
          "www-authenticate":
            "https://something.else/.well-known/oauth-protected-resource/mcp",
        },
      });
      mockResourceServer(f, "https://example.com", "/mcp");
      mockAuthorizationServer(f, DEFAULT_AUTHORIZATION_SERVER);
      ...
      const client = oauthClient(f.fetchHandler);
      await expect(client.fetch("https://example.com/mcp")).rejects.toThrow(
        "OAuth authentication required. Resource server url: https://something.else/mcp",
      );
    });
    ...
    it("should throw OAuthAuthenticationRequiredError with resource server url from MCP-spec www-authenticate header format", async () => {
      const f = fetchMock.createInstance().getOnce("https://example.com/mcp", {
        ...
        headers: {
          "www-authenticate":
            'Bearer resource_metadata="https://something.else/.well-known/oauth-protected-resource/mcp"',
        },
      });
      mockResourceServer(f, "https://example.com", "/mcp");
      mockAuthorizationServer(f, DEFAULT_AUTHORIZATION_SERVER);
    ...
    it("should store the url and resource server url for proxied requests", async () => {
      ...
      try {
        ...
        const authUrl = await client.makeAuthorizationUrl(
          oauthError.url,
    ...
    it("should send token in request to resource server if one exists in the DB", async () => {
      ...
      expect((mcpCall?.options?.headers as any)?.["authorization"]).toBe(
    ...
    it("should NOT send a stored token for the parent path if no token exists for the resource path", async () => {
      ...
      expect(
        (mcpCall?.options?.headers as any)?.["authorization"],
      ).toBeUndefined();
    });
    ...
    it("should use refresh token to get a new access token if the current one expires", async () => {
      ...
      const f = fetchMock
        ...
        .getOnce("https://example.com/mcp", {
          ...
          headers: {
            "www-authenticate":
              'Bearer error="invalid_grant", error_description="The token has expired"',
          },
        })
        .getOnce("https://example.com/mcp", 200);

      mockResourceServer(f, "https://example.com", "/mcp");
      mockAuthorizationServer(f, DEFAULT_AUTHORIZATION_SERVER).modifyRoute(
        `${DEFAULT_AUTHORIZATION_SERVER}/token`,
        {
          method: "post",
          response: {
            status: 200,
      ...
      expect(res.status).toBe(200);
      const tokenCall = f.callHistory.lastCall(
        `${DEFAULT_AUTHORIZATION_SERVER}/token`,
      );
    ...
    it("should throw if the token refresh fails", async () => {
      ...
      const f = fetchMock
        ...
        .getOnce("https://example.com/mcp", {
          ...
          headers: {
            "www-authenticate":
              'Bearer error="invalid_grant", error_description="The refresh token has expired"',
          },
        })
        .getOnce("https://example.com/mcp", 200);
      ...
      const client = oauthClient(f.fetchHandler, db);
      await expect(client.fetch("https://example.com/mcp")).rejects.toThrow(
        "Token Endpoint response (unexpected HTTP status code)",
      );
    });
  });
  ...
  describe(".getAuthorizationServer", () => {
    it("should make resource server PRM request", async () => {
      ...
      const client = oauthClient(f.fetchHandler);
      const res = await client.getAuthorizationServer(
        "https://example.com/mcp",
      );
      expect(res).toBeDefined();
      expect(res.issuer).toBe(DEFAULT_AUTHORIZATION_SERVER);
      expect(res.authorization_endpoint).toBe(
        `${DEFAULT_AUTHORIZATION_SERVER}/authorize`,
      );
      expect(res.registration_endpoint).toBe(
        `${DEFAULT_AUTHORIZATION_SERVER}/register`,
      );

      const prmCall = f.callHistory.lastCall(
    ...
    it("should not strip querystring for PRM request URL", async () => {
      ...
      const prmCall = f.callHistory.lastCall(
        "https://example.com/.well-known/oauth-protected-resource/mcp?test=1",
      );
      expect(prmCall).toBeDefined();
    });
    ...
    it("should try to request AS metadata from resource server if PRM doc cannot be found (non-strict mode)", async () => {
      ...
      const asCall = f.callHistory.lastCall(
    ...
    it("should throw if there is no way to find AS endpoints from resource server", async () => {
      ...
      const client = oauthClient(f.fetchHandler);
      await expect(
        client.getAuthorizationServer("https://example.com/mcp"),
      ).rejects.toThrow("unexpected HTTP status code");
    });
  });
  ...
  describe(".registerClient", () => {
    it("should configure safe metadata for public clients", async () => {
      ...
      await client.makeAuthorizationUrl(
        "https://example.com/mcp",
        "https://example.com/mcp",
      );
      const registerCall = f.callHistory.lastCall(
        `${DEFAULT_AUTHORIZATION_SERVER}/register`,
      );
      ...
      expect(body.client_name).toEqual(
        "OAuth Client for https://example.com/mcp/callback",
      );
    });
  ...
  describe(".makeAuthorizationUrl", () => {
    it("should make an authorization url", async () => {
      ...
      expect(authUrl.searchParams.get("redirect_uri")).toBe(
        "https://example.com/mcp/callback",
      );
  ...
  describe(".handleCallback", () => {
    ...
    it("should throw if no PKCE values found for state", async () => {
      ...
      await expect(client.handleCallback(authCallbackUrl)).rejects.toThrow(
        "No PKCE values found for state",
      );
    });
