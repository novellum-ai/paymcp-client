...
describe("OAuthAuthenticationRequiredError", () => {
  it("should return the same idempotencyKey for the same url, resource server url, and token", async () => {
    ...
    const error1 = await OAuthAuthenticationRequiredError.create(
      url,
      resourceServerUrl,
      token,
    );
    const error2 = await OAuthAuthenticationRequiredError.create(
      url,
      resourceServerUrl,
      token,
    );
  ...
  it("should return an idempotencyKey for undefined/null tokens", async () => {
    const url = "https://foo.com/mcp";
    const resourceServerUrl = "https://example.com";
    const error = await OAuthAuthenticationRequiredError.create(
      url,
      resourceServerUrl,
      undefined,
    );
  ...
  it("should return different idempotencyKeys for different tokens", async () => {
    ...
    const error3 = await OAuthAuthenticationRequiredError.create(
      url,
      resourceServerUrl,
      undefined,
    );
