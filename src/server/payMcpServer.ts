...
type BuildablePayMcpConfigFields =
  | "oAuthDb"
  | "oAuthClient"
  | "paymentServer"
  | "logger";
...
export const DEFAULT_CONFIG: Required<
  Omit<OptionalPayMcpConfig, BuildablePayMcpConfigFields>
> = {
  mountPath: "/",
...
export function buildServerConfig(args: PayMcpArgs): PayMcpConfig {
  ...
  const oAuthClient =
    withDefaults.oAuthClient ??
    new OAuthResourceClient({
      db: oAuthDb,
  ...
  const paymentServer =
    withDefaults.paymentServer ??
    new PayMcpPaymentServer(withDefaults.server, oAuthDb, logger);
...
export function payMcpServer(args: PayMcpArgs): Router {
  ...
  const payMcpMiddleware = async (
    ...
    try {
      ...
      const mcpRequests = await parseMcpRequests(
        config,
        requestUrl,
        req,
        req.body,
      ...
      res.on("finish", async () => {
        logger.debug(
          `Request finished ${user ? `for user ${user} ` : ""}- ${req.method} ${req.path}`,
        );
      });
      ...
      config.logger.error(
        `Critical error in paymcp middleware - returning HTTP 500. Error: ${error instanceof Error ? error.message : String(error)}`,
      );
