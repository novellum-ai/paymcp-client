...
describe("requirePayment", () => {
  it("should pass if there is money", async () => {
    const paymentServer = TH.paymentServer({
      charge: vi
        .fn()
        .mockResolvedValue({ success: true, requiredPaymentId: null }),
    });
    const config = TH.config({ paymentServer });
    await withPayMcpContext(
      config,
  ...
  it("should throw an error if there is no money", async () => {
    const paymentServer = TH.paymentServer({
      charge: vi.fn().mockResolvedValue({
        success: false,
        requiredPaymentId: "test-payment-request-id",
      }),
    });
    const config = TH.config({ paymentServer });
    await withPayMcpContext(
    ...
    await withPayMcpContext(
      ...
      async () => {
        try {
          ...
          expect(err.data.paymentRequestUrl).toBe(
            "https://example.com/payment-request/test-payment-request-id",
          );
        }
      },
    );
      ...
      async () => {
        try {
          await requirePayment({
            price: BigNumber(0.01),
            getExistingPaymentId: async () => "some-other-payment-id",
          });
        } catch (err: any) {
          expect(err).toBeInstanceOf(McpError);
          expect(err.data.paymentRequestId).toBe("some-other-payment-id");
          expect(err.data.paymentRequestUrl).toBe(
            "https://auth.paymcp.com/payment-request/some-other-payment-id",
          );
  ...
  it("should throw an error if the payment request fails", async () => {
    const paymentServer = TH.paymentServer({
      ...
      createPaymentRequest: vi
        .fn()
        .mockRejectedValue(new Error("Payment request failed")),
    });
    const config = TH.config({ paymentServer });
    await withPayMcpContext(
      config,
