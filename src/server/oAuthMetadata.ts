...
export function sendOAuthMetadata(
  res: ServerResponse,
  metadata: oauth.AuthorizationServer | null,
): boolean {
  if (!metadata) {
    return false;
  }
...
export async function getOAuthMetadata(
  ...
  if (isOAuthMetadataRequest(config, requestUrl)) {
    try {
      const authServer = await config.oAuthClient.authorizationServerFromUrl(
      ...
      return {
        ...
        introspection_endpoint_auth_methods_supported:
          authServer.introspection_endpoint_auth_methods_supported,
      ...
      config.logger.error(
        `Error fetching authorization server configuration from ${config.server}: ${error}`,
      );
      throw error;
    }
  }
...
function isOAuthMetadataRequest(
  config: PayMcpConfig,
  requestUrl: URL,
): boolean {
