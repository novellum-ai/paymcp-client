...
export function mcpElicitationRequiredErrorResponse({
  id = 1,
  message = "test error",
  elicitationId = "123",
  url = "https://paymcp.com/payment-request/123",
  data = {},
}: {
  id?: number;
  message?: string;
  ...
  const elicitationData = {
    elicitations: [{ mode: "url", elicitationId, url }],
  };
  return mcpErrorResponse({
    id,
    code,
    message,
    data: { ...data, ...elicitationData },
  });
}
